        ...
        case SPELLFAMILY_MAGE:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 13043:                     // Improved Fire Ward (Rank 2)
                {
                    ...
                    SpellModifier* mod = new SpellModifier(SPELLMOD_RESIST_MISS_CHANCE, SPELLMOD_FLAT, damage, m_spellInfo->Id, uint64(0x0000000000000008));
    ...
    if (!m_CastItem && skillId != SKILL_NONE)
    {
        ...
        if (uint32 pureSkillValue = player->GetPureSkillValue(skillId))
        {
            if (gameObjTarget)
            {
                ...
                if (!gameObjTarget->IsInSkillupLootList(player))
                {
                    gameObjTarget->AddToSkillupLootList(player);
...
void Spell::EffectTaunt(SpellEffectIndex eff_idx)
{
    ...
    {
        float currentTargetAggro = unitTarget->getThreatManager().getCurrentVictim()->getThreat();
        float currentCasterAggro = unitTarget->getThreatManager().getThreat(m_caster);
        float result = currentCasterAggro < currentTargetAggro ? currentTargetAggro - currentCasterAggro : 0;
        unitTarget->getThreatManager().addThreat(m_caster, result);
    }
